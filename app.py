...
# ==================== الترجمة (مختصرة للاختصار، نفس الترجمات السابقة) ====================
# (للاختصار، نفس الترجمات من الكود السابق)
translations = {
    "en": {
        ...
        "photo_camera": "📸 Take Photo (Camera)",
    ...
    "ar": {
        ...
        "photo_camera": "📸 التقاط صورة (كاميرا)",
    ...
    "sw": {
        ...
        "photo_camera": "📸 Piga Picha (Kamera)",
        "clear_photo": "Futa Picha"
    },
    "lg": {
        ...
        "photo_camera": "📸 Kuba Ekifananyi (Kamera)",
        "clear_photo": "Sangula Ekifananyi"
    },
    "rn": {
        ...
        "photo_camera": "📸 Fota Ekishushani (Kamera)",
        "clear_photo": "Hanagura Ekishushani"
    }
}
...
with tab1:
    ...
    with st.form(key="farmer_form"):
        ...
        # كاميرا حية بدلاً من رفع صورة
        photo = st.camera_input(t("photo_camera"))
...
with tab2:
    ...
    if district_farmers:
        with st.form(key="crop_form"):
            ...
            # كاميرا حية للمحصول
            crop_photo = st.camera_input(t("photo_camera"), key="crop_camera")
...
# ==================== TAB 3: عرض البيانات مع خريطة تفاعلية ====================
with tab3:
    ...
    # إنشاء خريطة Folium
    center_coords = DISTRICT_COORDS.get(st.session_state.district, [1.5, 32.0])
    m = folium.Map(location=center_coords, zoom_start=10, tiles='OpenStreetMap')
    
    # إضافة علامات للمزارعين
    for i, f in enumerate(district_farmers):
        # إضافة توزيع عشوائي بسيط لتجنب تراكم العلامات
        lat_offset = (i * 0.005) if i % 2 == 0 else -(i * 0.005)
        lon_offset = (i * 0.005) if i % 3 == 0 else -(i * 0.005)
        popup_text = f"<b>{f['name']}</b><br>📍 {f['village']}<br>📞 {f.get('phone', 'N/A')}"
        folium.Marker(
            location=[center_coords[0] + lat_offset, center_coords[1] + lon_offset],
            popup=popup_text,
            icon=folium.Icon(color='green', icon='leaf', prefix='fa')
        ).add_to(m)
    
    # إضافة علامة للمقاطعة نفسها
    folium.Marker(
        location=center_coords,
        popup=f"<b>{st.session_state.district} District</b><br>Pilot Area",
...
with tab4:
    ...
    **Features:**
    ...
    - Crop listing with live camera photos
