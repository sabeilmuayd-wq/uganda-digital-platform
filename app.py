...
def t(key):
    """دالة الترجمة - تستخدم في كل مكان"""
...
# ==================== صفحة المزارع ====================
    ...
    with col1:
        ...
        if st.button(f"{t('open_app')} →", key="soko", use_container_width=True):
            st.info("🔗 SOKO LINK: [your-soko-link.streamlit.app]")
...
# ==================== صفحة الحكومة ====================
    ...
    with col1:
        ...
        if st.button(f"{t('open_app')} →", key="datacollect", use_container_width=True):
            st.info("🔗 DataCollect UG: [your-datacollect.streamlit.app]")
    
    with col2:
        st.markdown(f"""
        <div class='service-card'>
            <div class='service-icon'>📈</div>
            <h3>{t('view_reports')}</h3>
            <p>{t('stats')}</p>
        </div>
        """, unsafe_allow_html=True)
        
        st.metric(t('total_complaints') if 'total_complaints' in dir() else "Complaints", len(load_data(COMPLAINTS_FILE)))
        st.metric(t('farmers_connected'), len([u for u in load_data(USERS_FILE) if u.get("type") == "farmer"]))
