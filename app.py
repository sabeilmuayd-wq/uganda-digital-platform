...
translations = {
    "en": {
        ...
        "quality_options": ["Low", "Medium", "High", "Premium"],
        "farmer_type_options": ["Smallholder", "Commercial", "Cooperative Member"],
        "crop_placeholder": "e.g., Bananas, Maize, Beans"
    },
    "ar": {
        ...
        "quality_options": ["منخفضة", "متوسطة", "عالية", "ممتازة"],
        "farmer_type_options": ["صغير", "تجاري", "عضو تعاونية"],
        "crop_placeholder": "مثال: موز، ذرة، فول"
    },
    "sw": {
        ...
        "quality_options": ["Chini", "Wastani", "Juu", "Bora"],
        "farmer_type_options": ["Mdogo", "Biashara", "Mwanachama wa Ushirika"],
        "crop_placeholder": "mfano: Ndizi, Mahindi, Maharage"
    },
    "lg": {
        ...
        "quality_options": ["Wansi", "Wakati", "Wagulu", "Kyezinga"],
        "farmer_type_options": ["Mutono", "Okugulisa", "Munnakibiina"],
        "crop_placeholder": "e.g. Ndiizi, Kayiso, Bbiri"
    },
    "rn": {
        ...
        "quality_options": ["Hakye", "Rikwera", "Rikuru", "Rikuru munonga"],
        "farmer_type_options": ["Mutono", "Okugurisa", "Omwezi w'ekibiina"],
        "crop_placeholder": "oku: Ebitooke, Emyoli, Ebiharagwe"
    }
}
...
with col1:
    all_farmers = load_data(FARMERS_FILE)
    st.metric("👨‍🌾 " + t("total_farmers"), len(all_farmers), delta="+2 this week")
with col2:
    all_crops = load_data(CROPS_FILE)
    st.metric("🌾 " + t("total_crops"), len(all_crops), delta="+5 this week")
with col3:
...
with tab2:
    ...
    if district_farmers:
        with st.form(key="crop_form"):
            ...
            quality = st.selectbox(t("quality"), t("quality_options"))
...
with tab3:
    ...
    if district_farmers:
        map_data = []
        coords = {"Kiryandongo": [1.9, 32.1], "Masindi": [1.7, 31.7], "Gulu": [2.8, 32.3]}
        ...
        fig = px.scatter_mapbox(
            ...
            hover_data={'village': True}, size=[5] * len(df_map),
            zoom=8, title=t("map_title")
        )
    ...
    with col1:
        ...
        st.markdown(f"**{t('avg_rating')}:** {stars} ({avg_rating:.1f}/5)")
    with col2:
...
with tab4:
    ...
    with col1:
        if st.button(t("export_csv"), key="csv_btn", use_container_width=True):
            ...
            st.download_button(label="Download CSV", data=csv, file_name=f"report_{datetime.now().strftime('%Y%m%d')}.csv", mime="text/csv", key="download_csv")
    with col2:
        if st.button(t("export_pdf"), key="pdf_btn", use_container_width=True):
            st.info("📄 PDF Report Ready. (In production, this would download a formatted report)")
    ...
    **Features:**
    - Farmer registration with National ID
